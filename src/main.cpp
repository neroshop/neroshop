...
namespace neroshop {
    ...
    static bool on_open() {
        ...
        if(!DB::create_config()) { // if it fails to create a config file
            if(!DB::load_config(DB::get_lua_state())) {return false;}// try loading the config file instead
        }
        //////////////////////////////////////////////////
        // cart.db
...
int main() {
    // start neroshop
    if(!neroshop::open()) {std::cout << "\033[1;91m" << "neroshop: failed to start" << "\033[0m" << std::endl;exit(0);}
	...
	//std::string text = "56WG1qKFWXtAuLzxB6RqLe17uwENMRgNHDqoc8mp29Q2dCcBZXmBEocMiMBLCd1x3Q2as9KuNCpK2KgBtcTWT1jA8EqkcoT";//"Hello, world!";              // User-supplied text
	//const QrCode::Ecc errCorLvl = QrCode::Ecc::HIGH;//QrCode::Ecc::LOW;  // Error correction level
	// Make and print the QR Code symbol
	//const QrCode qr = QrCode::encodeText(text.c_str(), errCorLvl);
	//printQr(qr);
	// texture export
	//Texture * texture = new Texture((unsigned char *)pixels.data(), 53, 53, 1, 4);
	//texture->export_png();
	// qr export
	//QR * qr_code = new QR("test_qr.png", 128, 3, text, true, QrCode::Ecc::HIGH);
	//if(!qr_code->to_png()) std::cout << "failed to write png\n";
	////////////////////////////////////////////////
	//std::string pw = "Password!2";
    //Validator::register_user("jack_sparrow3", pw, pw);
    // delete account
    /*User * user0 = nullptr;
    if(Validator::login("jack_sparrow3", pw)) {
        user0 = Buyer::on_login("jack_sparrow3");
    }
    static_cast<User*>(user0)->delete_account();*/
	////////////////////////////////////////////////
	//double dai_amount = 2.000000000000000000;
	//std::cout << dai_amount << " DAI = " << Converter::dai_to_xmr(dai_amount) << " XMR" << std::endl;
	///////////////////////////////////////////////
	// create a user
    User * user = nullptr;
    // on user login, copy data from db to application
    // if login fails, then try registering
    //Validator::register_user("laytera", "Password!23a", "Password!23a", "laytera@pm.me");//if(!Validator::login("layter", "Password!23")) { Validator::register_user("layter", "Password!23", "Password!23", ""); }
    // on_user_login -------------------------------
    /*std::string username = "";
    std::string password = "Password!23";
    bool logged_in = Validator::login(username, password);
    if(logged_in) {
        user = Seller::on_login(username);
        if(!user) user = Buyer::on_login(username);
    }
    if(!logged_in) {
        if(!user) user = new User("Guest");
        std::cout << "Hello, " << user->get_name() << std::endl;
    }*/
    //bool logged = Validator::login(username, password);
    //if(is_logged()) return;// if already logged, no need to login again for a second time
    //if(logged) { // if user successfully logs in, retrieve their data from the db and store it within the application
        //sqlite3_busy_timeout(db->get_handle(), 10000); // deal with db being locked
        //db->execute("PRAGMA journal mode=Wal");
        /*db->column("users", "ADD", "type", "TEXT"); // add column type to table Users
        db->update("users", "type", DB::to_sql_string("Seller"), "name=" + DB::to_sql_string(username)); // set user_type to seller
        */
    //}
    //db->select("users");
    //----------------------------------------------
    //Validator::register_user("kofi", "Password!234"); // can be registered a million times creating new hashes 'cause function does not check if username is taken [FIXED!]
    //Validator::register_user("jack", "Password!234");
    // ...
    // get data from Lua configfile
    //std::cout << "window size: " << Script::get_number(DB::get_lua_state(), "neroshop.window.width") << ", " << Script::get_number(DB::get_lua_state(), "neroshop.window.height") << std::endl;
    // colors
    /*std::cout << "\033[0;31m" << "red" << "\033[0m" << std::endl;
    std::cout << "\033[0;91m" << "bright red" << "\033[0m" << std::endl;
    std::cout << "\033[0;90m" << "bright black" << "\033[0m" << std::endl;
    std::cout << "\033[0;36m" << "cyan" << "\033[0m" << std::endl;
    std::cout << "\033[0;96m" << "bright cyan" << "\033[0m" << std::endl;
    std::cout << "\033[1;35;49m" << "magenta" << "\033[0m" << std::endl;
    std::cout << "\033[1;95;49m" << "bright magenta" << "\033[0m" << std::endl;
    std::cout << "\033[0;94m" << "bright blue" << "\033[0m" << std::endl;
    std::cout << "\033[0;37m" << "white" << "\033[0m" << std::endl;
    std::cout << "\033[0;97m" << "bright white" << "\033[0m" << std::endl;*/
	///////////////////////////////
	...
	//seller->list_item(ball, 50, Converter::usd_to_xmr(8.50)); //adds item to inventory
	//////////////
	//cart->add(ball);
	//cart->add(ring);
	//cart->add(game);
	...
	//db->select("Orders");
	//std::cout << "id of where name=kofi: " << db->get_column_integer("users", "id", "name='" + String::lower("KOFI") + "'") << std::endl;
	///db->drop("Orders");
	///////////////////////////////
    //double amount = 1;
    //std::string cur = "mxn";
    //std::cout << amount << " " << cur << " = " << Converter::to_xmr(amount, cur) << " xmr" << std::endl;
    ///////////////////////////////
...
*/
    ...
    ///////////////////////////////
    /*db->table("users");
    db->column("users", "ADD", "name", "VARCHAR(29)"); // 30 (index starts at 0)
    db->column("users", "ADD", "pw_hash", "VARCHAR(255)"); // 256 (index starts at 0)*/
    //db->insert("users", "name, pw_hash", ("'" + String::lower("layter") + "', " + "'$2a$12$Vs7S1KL45ya7TSxBTD9XEOx8NOHDjSMMusoJohVBRmV5pEwu.uFjW'") );
    // update "already set" values
    //db->update("users", "name", "'jack, mary, duke'", "id = 2"); // UPDATE Users SET name='layter' WHERE id=1
    //db->drop("users", "id=2"); // drop column where id=2  
    // select specific variables from table
    //db->select("users"); // shows all users // "SELECT * FROM table_name WHERE condition;"
    //db->select("users", "name", "id=2"); // "SELECT name FROM Users WHERE id=1;"// shows user's name where the id=2
    //db->drop_index("idx_users_hash");//index("idx_users_hash", "users", "pw_hash"); // prevents inserting a new user with a duplicate column
    // testing
    //std::cout << "SQLite: " << db->get_column_text("users", "name", "id=1") << std::endl;
    //db->execute("SELECT name FROM Users WHERE id=3");
    // close db when done
    //db->drop("users"); // deletes table from db
    db->close();
    delete db;
///////////////////////////////////////////////////////////////////////////////////////////////////////////////	
// Monero	
    ...
    if(network_type.empty()) {std::cout << "failed to get network type\nsetting default network type: stagenet" << std::endl; network_type = "stagenet";}
    // --rpc-bind-ip and --rpc-bind-port
    std::string ip = Script::get_string(DB::get_lua_state(), "neroshop.daemon.ip");
    if(ip.empty()) {std::cout << "failed to get ip\nsetting default ip: 127.0.0.1" << std::endl; ip = "127.0.0.1";}
    std::string port = Script::get_string(DB::get_lua_state(), "neroshop.daemon.port");
    if(port.empty()) {std::cout << "failed to get port\nsetting default port: 38081" << std::endl; port = "38081";}
    // --data-dir [path]
    ...
    if(data_dir.empty()) {std::cout << "failed to get data directory\nsetting default data directory: " << data_dir_default << std::endl; data_dir = data_dir_default;}
    ...
    if(Script::get_type(DB::get_lua_state(), "neroshop.daemon.confirm_external_bind") == "nil") { confirm_external_bind = false; if(ip == "0.0.0.0") {confirm_external_bind = true;} } // set default value if nil
    bool restricted_rpc = Script::get_boolean(DB::get_lua_state(), "neroshop.daemon.restricted_rpc");
    if(Script::get_type(DB::get_lua_state(), "neroshop.daemon.restricted_rpc") == "nil") {restricted_rpc = true;} // set default value if nil
    ...
    if(Script::get_type(DB::get_lua_state(), "neroshop.wallet.restore_height") == "number") { //std::cout << "restore height is a number\n";
        ...
        if(restore_height <= 0) {/*std::cout << "no user-specific restore_height\nsetting default restore_height: 0" << std::endl;*/ restore_height = 0;}
    }
    // --wallet-file
    ...
    // wallet_open -------------------------------------------------------------
    // if wallet_file is not empty, move it to the new location
    //wallet->open("/media/layter/Elements/layter/media/other/code/more/neroshop/my_wallet", "supersecretpassword123");
    /*if(!wallet_file.empty()) {
        wallet = new Wallet(); // wallet pw should be same as account pw??
        std::cout << "Please enter your wallet password: " << std::endl;
        std::string password;
        std::cin >> password; // get text from user_edit
        wallet->open(wallet_file.substr(0, wallet_file.find(".")), password);//"supersecretpassword123" // wallet_file.substr(0, wallet_file.find(".")) // removes ext
    }
    // if wallet_file is empty, the user will have to load it manually
    if(wallet_file.empty()) {
        wallet = new Wallet();
    #ifdef __gnu_linux__
        char file[1024];
        ...
        if(!File::exists(filename + ".keys")) {std::cout << "\033[1;31;49m" << "wallet not found" << "\033[0m" << std::endl;return 1;}// check if wallet file is valid (or exists)
        std::cout << "Please enter your wallet password: " << std::endl;
        std::string password;
        std::cin >> password;
        wallet->open(filename, password);//"supersecretpassword123" 
    }*/
    // check if wallet is view only
    //if(wallet->get_monero_wallet()->is_view_only ()) {std::cout << "wallet is view-only\n";}
    
    // daemon_connect ----------------------------------------------------------
    //wallet->daemon_connect(ip, port);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////	
	...
	while(window.is_open()) { // main thread
	    ...
	    ///////////////	    
		    ...
		    if(login_button->is_pressed() && !Message::is_visible()) {
		        ...
		        if(logged)//if(Validator::login(user_edit->get_text(), pw_edit->get_text()))
		        {
		            ...
		            db.close();
		            // create user
		            if(role_id == 0) {std::cout << "This user was not found in database"; neroshop::close();}
		    ...
		    if(upload_button->is_pressed() && !Message::is_visible() && !wallet_opened) {
		        ...
		        if(!wallet_file.empty()) {
                    ...
                    Message::get_button("ok")->show();    Message::get_edit()->set_text("supersecretpassword123");//temporary
	    ...
	    monero_icon->draw();
	    date_display->set_position(window.get_client_width() - date_display->get_width(), window.get_client_height() - date_display->get_height());
	    date_display->set_string(Validator::get_date("%Y-%m-%d  %l:%M:%S %p")); date_display->draw();
		
		if(Mouse::is_over(cart_icon->get_image()->get_rect())) cart_icon->get_image()->set_color(242, 100, 17); else {cart_icon->get_image()->set_color(default_icon_color);}
		if(Mouse::is_over(search_icon->get_rect())) search_icon->get_image()->set_color(242, 100, 17); else search_icon->get_image()->set_color(default_icon_color);
		...
		if(Mouse::is_over(user_icon->get_rect())) user_icon->get_image()->set_color(242, 100, 17); else user_icon->get_image()->set_color(default_icon_color);
		if(Mouse::is_over(order_icon->get_rect())) order_icon->get_image()->set_color(242, 100, 17); else order_icon->get_image()->set_color(default_icon_color);
	    ...
	    window.update(); // part of main thread
	} // while window.is_open() // main thread
	wallet->daemon_close(); // kill daemon
	window.destroy();
	//////////////////////////////////////////////////////////////////////////////////////////////////////////
// wait for the next block to be added to the chain
//uint64_t next_height = wallet_obj->wait_for_next_block();
...
config.m_address = wallet_obj->get_address(1, 0); // wallet_random
config.m_amount = 0;
config.m_relay = true;
shared_ptr<monero_tx_wallet> sent_tx = wallet_obj->create_tx(config);
...
// daemon:
cd '/media/layter/Elements/layter/wallet/ubuntu/monero-wallet-cli/monero-x86_64-linux-gnu-v0.17.1.9' #/path/to/monerod
./monerod --data-dir /media/layter/Elements/layter/wallet/other/chain --stagenet  #/path/to/lmdb
cd  ./monerod --data-dir '/media/layter/Extreme SSD/layter/chain/wallet'
./monerod --data-dir '/media/layter/Extreme SSD/layter/chain' --stagenet

// wallet:
cd '/media/layter/Elements/layter/wallet/ubuntu/monero-wallet-cli/monero-x86_64-linux-gnu-v0.17.1.9'
./monero-wallet-cli --wallet-file '/media/layter/Elements/layter/media/other/code/more/neroshop/MyWalletRestored.keys' --stagenet
