...
void neroshop::Seller::list_item(const neroshop::Item& item, unsigned int stock_qty, double sales_price, std::string currency, 
	...
	db.insert("inventory", "item_id, seller_id, stock_qty, seller_price, currency, seller_discount, discount_qty, condition", 
	    std::to_string(item.get_id()) + ", " + std::to_string(get_id()) + ", " + std::to_string(stock_qty) + ", " + std::to_string(sales_price) + ", " + DB::Sqlite3::to_sql_string(String::lower(currency)) + ", " + std::to_string(discount) + ", " + std::to_string(discounted_items) + ", " + DB::Sqlite3::to_sql_string(condition));
    ...
    DB::Psql::get_singleton()->execute("SAVEPOINT before_seller_item_listing_savepoint;");
    // create table: inventory
    if(!DB::Psql::get_singleton()->table_exists("inventory")) { // if(!db.table_exists(""))
	    DB::Psql::get_singleton()->create_table("inventory"); // inventory_id will be auto generated (primary key)
	    DB::Psql::get_singleton()->add_column("inventory", "item_id", "integer REFERENCES item(id) ON DELETE CASCADE"); // if an item is deleted, it no longer exists so CASCADE will remove said item from the inventory as well // CASCADE isn't working when I drop table item? :O
	    DB::Psql::get_singleton()->add_column("inventory", "seller_id", "integer REFERENCES users(id) ON DELETE CASCADE"); // store_id or seller_id or vendor_id // if seller is deleted, so will the seller's listing (there's no inventory without the seller)
	    DB::Psql::get_singleton()->add_column("inventory", "stock_qty", "integer"); //db.column("inventory", "ADD", "stock_available", "TEXT"); // or in_stock
	    DB::Psql::get_singleton()->add_column("inventory", "seller_price", "numeric(20, 12)");//"decimal(12,2)"); // price_per_item_unit //db.index("idx_item_ids", "inventory", "item_id"); actually, don't make item_id unique as multiple sellers could be selling the same item //db.execute("CREATE UNIQUE INDEX idx_seller_ids ON Inventory (seller_id);"); // sellers can have multiple items so seller_id should not be unique
	    DB::Psql::get_singleton()->add_column("inventory", "currency", "text"); // seller's currency of choice, which will be converted to xmr
	    DB::Psql::get_singleton()->add_column("inventory", "seller_discount", "numeric(20, 12)");//"real"); // 20 - 12 = 8 total digits (on left side / whole number side) // 12 decimals // seller_discount per discounted_items // or discount_per_x_items // money should not be used when dealing with multiple currencies, instead use numeric as suggested by postgresql.org // force 2 units precision for fiat currencies: 
	    DB::Psql::get_singleton()->add_column("inventory", "discount_qty", "integer"); // number of items that the discount will apply to // or discounted_items // for every x item, you get a discount (seller_discount)
	    DB::Psql::get_singleton()->add_column("inventory", "discount_times", "integer"); // number of times the discount can be used (in a single order)
	    DB::Psql::get_singleton()->add_column("inventory", "discount_expiry", "timestamptz DEFAULT NULL"); // will be in UTC format // date the discount expires
	    DB::Psql::get_singleton()->add_column("inventory", "condition", "text"); // seller_condition for each item
	}    
    // to prevent duplicating item_id that is being sold by the same seller_id (a seller cannot list the same item twice, except change the stock amount)
    int item_id = DB::Psql::get_singleton()->get_integer_params("SELECT item_id FROM inventory WHERE item_id = $1 AND seller_id = $2", { std::to_string(item.get_id()), std::to_string(get_id()) });
	if(item_id == item.get_id()) { 
	    neroshop::print("\033[1;33mYou have already listed this item (id: " + std::to_string(item_id) + ")\033[0m"); 
	    DB::Psql::get_singleton()->execute("ROLLBACK TO before_seller_item_listing_savepoint;");
	    //DB::Psql::get_singleton()->finish();
	    return;
	}	
	// convert localtime to universal time
	//std::cout << "discount_exp_date to UTC: " << DB::Psql::get_singleton()->localtimestamp_to_utc(discount_expiry) << std::endl;
	// SELECT TO_TIMESTAMP('2021 12 10', 'YYYY-MM-DD HH24:MI:SS');
	// SELECT TO_TIMESTAMP('2021-12-10', 'YYYY-MM-DD HH24:MI:SS');
	// SELECT TO_CHAR(now(), 'YYYY-MM-DD HH24:MI:SS') ;
	// insert item in inventory
	DB::Psql::get_singleton()->execute_params("INSERT INTO inventory (item_id, seller_id, stock_qty, seller_price, currency, seller_discount, discount_qty, discount_times, discount_expiry, condition) "
...
void neroshop::Seller::load_customer_orders() {
    ...
    //DB::Psql::get_singleton()->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest");
    /*if(!DB::Psql::get_singleton()->table_exists("order_item")) return;
    // check for orders made by customers
    // get last inserted order item
    int last_order_item = DB::Psql::get_singleton()->get_last_id("order_item");
    // get all order_items
    int customer_order_item_count = DB::Psql::get_singleton()->get_integer_params("SELECT COUNT(*) FROM order_item WHERE seller_id = $1", { std::to_string(get_id()) });
    //std::cout << "number of items that customers have ordered from you: " << customer_order_item_count << std::endl;
    if(customer_order_item_count < 1) neroshop::print("No buyer has ordered an item from you yet");
    if(customer_order_item_count > 0) {
        for(unsigned int i = 1; i <= last_order_item; i++) {
            //if order_item's order_id is duplicated, then it means there are multiple unique items in the order
            unsigned int order_item_id = DB::Psql::get_singleton()->get_integer_params("SELECT id FROM order_item WHERE id = $1 AND seller_id = $2", { std::to_string(i),  std::to_string(get_id()) });
            if(order_item_id == 0) continue; // skip 0's
            // get order_id of the order_item
            unsigned int order_id = DB::Psql::get_singleton()->get_integer_params("SELECT order_id FROM order_item WHERE id = $1 AND seller_id = $2", { std::to_string(i), std::to_string(get_id()) });//if(order_id == 0) continue; // skip 0's
            // store order_ids if not already stored
            if(std::find(customer_order_list.begin(), customer_order_list.end(), order_id) == customer_order_list.end()) {
                customer_order_list.push_back(order_id); //Order * order = new Order(order_id);//customer_order_list.push_back(order);
                neroshop::print("Customer order (id: " + std::to_string(order_id) + ") has been loaded");
        ...
        #ifdef NEROSHOP_DEBUG0
            // get items in the order_item table
            unsigned int item_id = DB::Psql::get_singleton()->get_integer_params("SELECT item_id FROM order_item WHERE id = $1 AND seller_id = $2", { std::to_string(i), std::to_string(get_id()) });
            unsigned int item_qty = DB::Psql::get_singleton()->get_integer_params("SELECT item_qty FROM order_item WHERE id = $1 AND seller_id = $2", { std::to_string(i), std::to_string(get_id()) });
            Item item(item_id); // item obj will die at the end of this scope
            std::cout << "You've received an order (id: " << order_id << ") from a customer " 
            << "containing items: " << item.get_name() << " (id: " << item_id << ", qty: " << item_qty << ")" << std::endl;
        #endif    
    ...
    int seller_customer_order_item_count = DB::Psql::get_singleton()->get_integer_params("SELECT COUNT(*) FROM order_item WHERE seller_id = $1", { std::to_string(get_id()) });
    if(seller_customer_order_item_count < 1) {neroshop::print("No buyer has ordered an item from you yet"); /*DB::Psql::get_singleton()->finish();*/ return;}    
    ...
    } //DB::Psql::get_singleton()->get_integer_params("SELECT item_id FROM order_item WHERE id = $1 AND seller_id = $2", { std::to_string(i), std::to_string(get_id()) });
    ////////////////////////////////    
    /*DB::Psql::get_singleton()->finish();*/
    ////////////////////////////////    
}
...
void neroshop::Seller::update_customer_orders() { // this function is faster (I think) than load_customer_orders()
    ...
    for(int i = 0; i < rows; i++) {
        ...
        if(std::find(customer_order_list.begin(), customer_order_list.end(), customer_order_id) == customer_order_list.end()) {
            ...
            bool is_pending = DB::Psql::get_singleton()->get_integer_params("SELECT id FROM orders WHERE id = $1 AND status = $2", { std::to_string(customer_order_id), "Pending" });
            if(is_pending) {
...
void neroshop::Seller::set_stock_quantity(unsigned int item_id, unsigned int stock_qty) {
    ...
    ////////////////////////////////
    //DB::Psql::get_singleton()->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest"); 
    DB::Psql::get_singleton()->execute_params("UPDATE inventory SET stock_qty = $1 WHERE item_id = $2 AND seller_id = $3", { std::to_string(stock_qty), std::to_string(item_id), std::to_string(get_id()) });
    /*DB::Psql::get_singleton()->finish();*/
    ////////////////////////////////
}
////////////////////
void neroshop::Seller::set_stock_quantity(const neroshop::Item& item, unsigned int stock_qty) {
    ...
    ////////////////////////////////
    //DB::Psql::get_singleton()->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest"); 
    DB::Psql::get_singleton()->execute_params("UPDATE inventory SET stock_qty = $1 WHERE item_id = $2 AND seller_id = $3", { std::to_string(stock_qty), std::to_string(item.get_id()), std::to_string(get_id()) });
    /*DB::Psql::get_singleton()->finish();*/
    ////////////////////////////////	
}
////////////////////
...
unsigned int neroshop::Seller::get_good_ratings() const {
    ...
    ////////////////////////////////
    //DB::Psql::get_singleton()->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest");	
	if(!DB::Psql::get_singleton()->table_exists("seller_ratings")) {/*DB::Psql::get_singleton()->finish();*/ return 0;}
    unsigned int good_ratings_count = DB::Psql::get_singleton()->get_integer_params("SELECT COUNT(score) FROM seller_ratings WHERE seller_id = $1 AND score = $2", { std::to_string(get_id()), std::to_string(1) });
	/*DB::Psql::get_singleton()->finish();*/
	return good_ratings_count;
	////////////////////////////////
}
unsigned int neroshop::Seller::get_bad_ratings() const {
    ...
    //DB::Psql::get_singleton()->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest");	
	if(!DB::Psql::get_singleton()->table_exists("seller_ratings")) {/*DB::Psql::get_singleton()->finish();*/ return 0;}
    unsigned int bad_ratings_count = DB::Psql::get_singleton()->get_integer_params("SELECT COUNT(score) FROM seller_ratings WHERE seller_id = $1 AND score = $2", { std::to_string(get_id()), std::to_string(0) });    	
	/*DB::Psql::get_singleton()->finish();*/
	return bad_ratings_count;
	////////////////////////////////    
}
...
unsigned int neroshop::Seller::get_ratings_count() const {
    ...
    //DB::Psql::get_singleton()->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest");	
    if(!DB::Psql::get_singleton()->table_exists("seller_ratings")) {/*DB::Psql::get_singleton()->finish();*/ return 0;}
    unsigned int ratings_count = DB::Psql::get_singleton()->get_integer_params("SELECT COUNT(*) FROM seller_ratings WHERE seller_id = $1", { std::to_string(get_id()) });    
	/*DB::Psql::get_singleton()->finish();*/
    return ratings_count;
    ////////////////////////////////
}
...
unsigned int neroshop::Seller::get_reputation() const {
	...
	if(!DB::Psql::get_singleton()->table_exists("seller_ratings")) {/*DB::Psql::get_singleton()->finish();*/ return 0;}
    unsigned int ratings_count = DB::Psql::get_singleton()->get_integer_params("SELECT COUNT(*) FROM seller_ratings WHERE seller_id = $1", { std::to_string(get_id()) });
    if(ratings_count == 0) {/*DB::Psql::get_singleton()->finish();*/ return 0;} // seller has not yet been rated so his or her reputation will be 0%
    // get seller's good ratings
    unsigned int good_ratings = DB::Psql::get_singleton()->get_integer_params("SELECT COUNT(score) FROM seller_ratings WHERE seller_id = $1 AND score = $2", { std::to_string(get_id()), std::to_string(1) });
    /*DB::Psql::get_singleton()->finish();*/
    // calculate seller reputation
    double reputation = (good_ratings / static_cast<double>(ratings_count)) * 100;
...
std::vector<int> neroshop::Seller::get_pending_customer_orders() {
    ...
    for(int i = 0; i < customer_order_list.size(); i++) {
         int pending_orders = db.get_column_integer("orders", "id", "id = " + std::to_string(customer_order_list[i]) + " AND status = " + DB::Sqlite3::to_sql_string("Pending"));
         if(pending_orders != 0) {
    ...
    for(int i = 0; i < customer_order_list.size(); i++) {
         int pending_orders = DB::Psql::get_singleton()->get_integer_params("SELECT id FROM orders WHERE id = $1 AND status = $2", { std::to_string(customer_order_list[i]), "Pending" });
         if(pending_orders != 0) {
             pending_order_list.push_back(customer_order_list[i]);
...
bool neroshop::Seller::has_listed(unsigned int item_id) const {
    ...
    ////////////////////////////////
    //DB::Psql::get_singleton()->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest");	
	int id = DB::Psql::get_singleton()->get_integer_params("SELECT item_id FROM inventory WHERE item_id = $1 AND seller_id = $2", { std::to_string(item_id), std::to_string(get_id()) });
	/*DB::Psql::get_singleton()->finish();*/
	////////////////////////////////
	return (id != 0);
}
...
neroshop::User * neroshop::Seller::on_login(const std::string& username) { // assumes user data already exists in database
    ...
    dynamic_cast<Seller *>(user)->set_id(db.get_column_integer("users", "id", "name = " + DB::Sqlite3::to_sql_string(username)));
    ...
    // postgresql
    ////////////////////////////////
    //DB::Psql::get_singleton()->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest");//DB::Psql::get_singleton()->connect;//DB::Psql::get_singleton()->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest");
