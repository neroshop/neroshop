...
void neroshop::Order::create_guest_order(unsigned int user_id, const std::string& shipping_address, std::string contact_info) // order: order_id, [order_date], product, SKU, quantity, price (subtotal), discount (optional), shipping_cost/estimated_delivery, payment method:monero[xmr], total
{
    ...
    DB::Postgres::get_singleton()->execute_params("UPDATE orders SET status = $1 WHERE id = $2", { get_status_string(), std::to_string(order_id) });
    // update order details - converts seller's currency of choice to xmr, the moment you create an order
    DB::Postgres::get_singleton()->execute_params("UPDATE orders SET subtotal = $1 WHERE id = $2", { std::to_string(neroshop::Converter::to_xmr(subtotal, seller_currency)), std::to_string(order_id) });
    DB::Postgres::get_singleton()->execute_params("UPDATE orders SET discount = $1 WHERE id = $2", { std::to_string(neroshop::Converter::to_xmr(discount, seller_currency)), std::to_string(order_id) });
    DB::Postgres::get_singleton()->execute_params("UPDATE orders SET shipping_cost = $1 WHERE id = $2", { std::to_string(neroshop::Converter::to_xmr(shipping_cost, seller_currency)), std::to_string(order_id) });
    double total = (subtotal - discount) + shipping_cost;
    DB::Postgres::get_singleton()->execute_params("UPDATE orders SET total = $1 WHERE id = $2", { std::to_string(neroshop::Converter::to_xmr(total, seller_currency)), std::to_string(order_id) });
    // display order details
    std::string currency = Script::get_string(neroshop::get_lua_state(), "neroshop.currency");
    if(currency.empty() || !neroshop::Converter::is_supported_currency(currency)) currency = "usd"; // default //neroshop::Converter::from_xmr(neroshop::Converter::to_xmr(, seller_currency), currency);
    neroshop::print("Sit tight as we notify the seller(s) about your order.");
