...
void neroshop::Order::create_guest_order(unsigned int user_id, const std::string& shipping_address, std::string contact_info) // order: order_id, [order_date], product, SKU, quantity, price (subtotal), discount (optional), shipping_cost/estimated_delivery, payment method:monero[xmr], total
{
    ...
    if(!DB::Psql::get_singleton()->table_exists("orders")) { 
        DB::Psql::get_singleton()->create_table("orders");
        // order-specific
        DB::Psql::get_singleton()->add_column("orders", "user_id", "integer REFERENCES users(id)"); // customer that is placing the order
        DB::Psql::get_singleton()->add_column("orders", "date", "timestamptz DEFAULT CURRENT_TIMESTAMP"); // date and time order was created
        DB::Psql::get_singleton()->add_column("orders", "status", "text"); // order status // for each individual item // you don't need all these once you have the item_id (qty, price, weight, sku_code, etc.)        
        DB::Psql::get_singleton()->add_column("orders", "weight", "real"); // total weight of all items
        DB::Psql::get_singleton()->add_column("orders", "subtotal", "numeric(20, 12)"); // 20 - 12 = 8 total digits (on left side / whole number side) // 12 decimals // monero supply is 18 million so that is an 8 digit-number // price of all items in cart (combined)
        DB::Psql::get_singleton()->add_column("orders", "discount", "numeric(20, 12)"); // discount from coupon (optional)(price will be reduced by discount)
        //DB::Psql::get_singleton()->column("orders", "ADD", "shipping_address", "TEXT"); // will not be saved in db (for the sake of user's privacy), but will be sent to the seller // shipping_address - name, address, city, region, postal code, country
        //DB::Psql::get_singleton()->column("orders", "ADD", "shipping_method", "TEXT"); // 1 day, 2 day, 3 day, 1 week, etc.
        DB::Psql::get_singleton()->add_column("orders", "shipping_cost", "numeric(20, 12)");
        DB::Psql::get_singleton()->add_column("orders", "total", "numeric(20, 12)"); // total cost of all items plus shipping costs (taxes don't exist in the cryptocurrency world except in the form of tx fees)
        DB::Psql::get_singleton()->add_column("orders", "payment_method", "text"); //  cash, card, crypto, etc.
        DB::Psql::get_singleton()->add_column("orders", "currency", "text"); //  xmr, btc, usd, etc.
        DB::Psql::get_singleton()->add_column("orders", "notes", "text"); // must be encrypted since storing this in db
        //db.column("orders", "ADD", "", ""); // monero_address (of receipient)?
        //db.index("idx_orders_code", "orders", "code");// code can be sku, upc, ean, etc. //db.execute("CREATE UNIQUE INDEX idx_orders_sku ON Orders (sku);");
    }
    // before we create the order, lets create a savepoint (just in case the order fails or we screw something up)
    DB::Psql::get_singleton()->execute("SAVEPOINT before_order_creation_savepoint;");
    // set order date (timestamp) to the current date (timestamp) // timestamp includes both date and time instead of the time only
    std::string date = DB::Psql::get_singleton()->get_text("SELECT CURRENT_TIMESTAMP;"); // SELECT NOW(), CURRENT_TIMESTAMP; // both now() and current_timestamp are the same thing and both include the timezone: https://dba.stackexchange.com/questions/63548/difference-between-now-and-current-timestamp    // NOW() is postgresql's version; CURRENT_TIMESTAMP is an SQL-standard
    // insert new order
    int order_id = DB::Psql::get_singleton()->get_integer_params("INSERT INTO orders (user_id, date, status, weight, subtotal, discount, shipping_cost, total, payment_method, currency, notes) "
    ...
    /////////////////////////////////////////
    if(!DB::Psql::get_singleton()->table_exists("order_item")) {
        DB::Psql::get_singleton()->create_table("order_item");
        DB::Psql::get_singleton()->add_column("order_item", "order_id", "integer REFERENCES orders(id) ON DELETE CASCADE");  // the order id in which the order_item belongs to
        DB::Psql::get_singleton()->add_column("order_item", "item_id", "integer REFERENCES item(id)");     // the order_item's item_id
        DB::Psql::get_singleton()->add_column("order_item", "seller_id", "integer REFERENCES users(id)");  // the seller that is selling the item_id                
        DB::Psql::get_singleton()->add_column("order_item", "item_qty", "integer");  // the quantity of the item the buyer wishes to buy
        DB::Psql::get_singleton()->add_column("order_item", "item_price", "numeric(20, 12)");   // might not need the item price at all //db.column("order_item", "ADD", "", "");
    }
    ...
    for(int i = 0; i < cart.get_contents_count(); i++) {
        neroshop::Item * item = cart.get_item(i);
        unsigned int item_id  = cart.get_item(i)->get_id();
        std::string item_name = DB::Psql::get_singleton()->get_text_params("SELECT name FROM item WHERE id = $1", { std::to_string(item_id) });//cart.get_item(i)->get_name();
        unsigned int item_qty = cart.get_item(i)->get_quantity();//DB::Psql::get_singleton()->get_integer("SELECT item_qty FROM cart or cart_item WHERE item_id = $1", { std::to_string(item_id) });
        // if seller_id is not specified (0), then choose a random seller who is selling the same product, but it MUST be in stock!!
        int seller_id = DB::Psql::get_singleton()->get_integer_params("SELECT seller_id FROM inventory WHERE item_id = $1 AND stock_qty > 0", { std::to_string(item_id) });
        if(seller_id == 0) { std::cout << "item seller not found"; DB::Psql::get_singleton()->execute("ROLLBACK TO before_order_creation_savepoint;"); /*DB::Psql::get_singleton()->finish();*/ return; }
        // if the buyer is also the seller XD
        if(user_id == seller_id) {neroshop::print("You cannot buy from yourself", 1); DB::Psql::get_singleton()->execute("ROLLBACK TO before_order_creation_savepoint;"); /*DB::Psql::get_singleton()->finish();*/ return;} // go back to savepoint, before the order was even create to prevent us from inserting a new order
        // get the currency that item is priced in
        seller_currency = DB::Psql::get_singleton()->get_text_params("SELECT currency FROM inventory WHERE item_id = $1 AND seller_id = $2", { std::to_string(item_id), std::to_string(seller_id) });
        // get seller_price
        // if seller does not specify a price for their item, set the item price to its original price
        double seller_price = DB::Psql::get_singleton()->get_double_params("SELECT seller_price FROM inventory WHERE item_id = $1 AND seller_id = $2", { std::to_string(item_id), std::to_string(seller_id) });//NEROSHOP_TAG_OUT std::cout << "Seller's price for item (id: " << item_id << ") per unit is: " << seller_price << " " << seller_currency << std::endl;
        double item_price = (seller_price > 0.00) ? seller_price : item->get_price(); // set the item price to the seller's price
        // calculate subtotal (price of all items combined)
        subtotal += item_qty * item_price;
        // get seller discount
        // if seller does not specify a discount for their item, set the item discount to its original discount which is 0
        double seller_discount = DB::Psql::get_singleton()->get_double_params("SELECT seller_discount FROM inventory WHERE item_id = $1 AND seller_id = $2", { std::to_string(item_id), std::to_string(seller_id) });
        unsigned int discounted_items = DB::Psql::get_singleton()->get_integer_params("SELECT discount_qty FROM inventory WHERE item_id = $1 AND seller_id = $2", { std::to_string(item_id), std::to_string(seller_id) });
        unsigned int discount_times = DB::Psql::get_singleton()->get_integer_params("SELECT discount_times FROM inventory WHERE item_id = $1 AND seller_id = $2", { std::to_string(item_id), std::to_string(seller_id) });
        if(seller_discount > 0.00) { // && discount_times > 0
            ...
            std::string discount_exp_date = DB::Psql::get_singleton()->get_text_params("SELECT discount_expiry FROM inventory WHERE item_id = $1 AND seller_id = $2", { std::to_string(item_id), std::to_string(seller_id) }); 
            // convert discount exp_date from localtime to utc // then get the time difference between now and the expiration date
            int timestamp_since_exp_date = std::stoi(DB::Psql::get_singleton()->get_text_params("SELECT extract (second from (timezone('utc', now()) - $1))", { DB::Psql::get_singleton()->localtimestamp_to_utc(discount_exp_date) }));//stoi(DB::Psql::get_singleton()->get_text_params("SELECT NOW() - $1::timestamptz", { discount_exp_date }));//DB::Psql::get_singleton()->get_text_params("SELECT DATE_TRUNC('second', timezone('utc', now()) - $1)", { DB::Psql::get_singleton()->localtimestamp_to_utc(discount_exp) })//prints HH:MM:SS
        ...
        // get condition of item based on seller
        std::string item_condition = DB::Psql::get_singleton()->get_text_params("SELECT condition FROM inventory WHERE item_id = $1 AND seller_id = $2", { std::to_string(item_id), std::to_string(seller_id) });
        // check again if item is still in stock
        bool item_in_stock = DB::Psql::get_singleton()->get_integer_params("SELECT stock_qty FROM inventory WHERE item_id = $1 AND stock_qty > 0", { std::to_string(item_id) });
        if(!item_in_stock) {
            neroshop::print("[error]: order failed  [reason]: The following item is out of stock: " + item_name);
            // set order status to failed
            set_status(order_status::failed);
            DB::Psql::get_singleton()->execute_params("UPDATE orders SET status = $1 WHERE id = $2", { get_status_string(), std::to_string(order_id) });
            // revert everything to the point before we created the order
            neroshop::print("reverting order creation ...", 3);
            DB::Psql::get_singleton()->execute("ROLLBACK TO before_order_creation_savepoint;");
            /*DB::Psql::get_singleton()->finish();*/
            return; // exit function //continue; // skip this item
        }
        // add each item to the same order_id
        DB::Psql::get_singleton()->execute_params("INSERT INTO order_item (order_id, item_id, seller_id, item_qty, item_price) "
        ...
        // reduce stock_qty of each purchased item (subtract stock_qty by item_qty that buyer is purchasing)
        int stock_qty = DB::Psql::get_singleton()->get_integer_params("SELECT stock_qty FROM inventory WHERE item_id = $1 AND seller_id = $2", { std::to_string(item_id), std::to_string(seller_id) });//std::cout << "stock_qty of item BEFORE deletion: " << stock_qty << std::endl; 
        DB::Psql::get_singleton()->execute_params("UPDATE inventory SET stock_qty = $1 WHERE item_id = $2 AND seller_id = $3", { std::to_string(stock_qty - item_qty), std::to_string(item_id), std::to_string(seller_id) });//std::cout << "stock_qty of item AFTER deletion: " << db.get_column_integer("inventory", "stock_qty", "item_id = " + std::to_string(item->get_id()) + " AND seller_id = " + std::to_string(seller_id)) << std::endl;
        // If stock qty goes to 0, delete this item from inventory (row) - bad idea because once an item is listed by a seller then the listing is permanent unless the seller's account is deleted or the manufacture of the item has discontinued
        //stock_qty = DB::Psql::get_singleton()->get_integer_params("SELECT stock_qty FROM inventory WHERE item_id = $1 AND seller_id = $2", { std::to_string(item_id), std::to_string(seller_id) });
        //if(stock_qty <= 0) DB::Psql::get_singleton()->execute_params("DELETE FROM inventory WHERE stock_qty = $1", { std::to_string(0) });
    }
    // print order message
    neroshop::print("Thank you for using neroshop.");
    neroshop::io_write("You have ordered: ");
    for(int i = 0; i < cart.get_contents_count(); i++) {
        neroshop::Item * item = cart.get_item(i);
        std::string item_name = DB::Psql::get_singleton()->get_text_params("SELECT name FROM item WHERE id = $1", { std::to_string(item->get_id()) });//unsigned int item_qty = db.get_column_integer("Cart", "item_qty", "item_id = " + std::to_string(item_id));
    ...
    DB::Psql::get_singleton()->execute_params("UPDATE orders SET status = $1 WHERE id = $2", { get_status_string(), std::to_string(order_id) });
    // update order details - converts seller's currency of choice to xmr, the moment you create an order
    DB::Psql::get_singleton()->execute_params("UPDATE orders SET subtotal = $1 WHERE id = $2", { std::to_string(neroshop::Converter::to_xmr(subtotal, seller_currency)), std::to_string(order_id) });
    DB::Psql::get_singleton()->execute_params("UPDATE orders SET discount = $1 WHERE id = $2", { std::to_string(neroshop::Converter::to_xmr(discount, seller_currency)), std::to_string(order_id) });
    DB::Psql::get_singleton()->execute_params("UPDATE orders SET shipping_cost = $1 WHERE id = $2", { std::to_string(neroshop::Converter::to_xmr(shipping_cost, seller_currency)), std::to_string(order_id) });
    double total = (subtotal - discount) + shipping_cost;
    DB::Psql::get_singleton()->execute_params("UPDATE orders SET total = $1 WHERE id = $2", { std::to_string(neroshop::Converter::to_xmr(total, seller_currency)), std::to_string(order_id) });
    // display order details
    std::string currency = Script::get_string(neroshop::get_lua_state(), "neroshop.currency");
    if(currency.empty() || !neroshop::Converter::is_supported_currency(currency)) currency = "usd"; // default //neroshop::Converter::from_xmr(neroshop::Converter::to_xmr(, seller_currency), currency);
    neroshop::print("Sit tight as we notify the seller(s) about your order.");
...
////////////////////
void neroshop::Order::create_user_order(unsigned int user_id, const std::string& shipping_address, std::string contact_info) {
    ...
    if(!DB::Psql::get_singleton()->table_exists("orders")) { 
        DB::Psql::get_singleton()->create_table("orders");
        DB::Psql::get_singleton()->execute("ALTER TABLE orders ADD COLUMN IF NOT EXISTS user_id integer REFERENCES users(id);"); // customer that is placing the order
        DB::Psql::get_singleton()->execute("ALTER TABLE orders ADD COLUMN IF NOT EXISTS order_date timestamptz DEFAULT CURRENT_TIMESTAMP;"); // date and time order was created
        DB::Psql::get_singleton()->execute("ALTER TABLE orders ADD COLUMN IF NOT EXISTS status text;");
        DB::Psql::get_singleton()->execute("ALTER TABLE orders ADD COLUMN IF NOT EXISTS weight real;"); // total weight of all items combined
        DB::Psql::get_singleton()->execute("ALTER TABLE orders ADD COLUMN IF NOT EXISTS subtotal numeric(20, 12);"); // price of all items in cart (combined)
        DB::Psql::get_singleton()->execute("ALTER TABLE orders ADD COLUMN IF NOT EXISTS discount numeric(20, 12);");////DB::Psql::get_singleton()->execute("ALTER TABLE orders ADD COLUMN IF NOT EXISTS shipping_address text;");////DB::Psql::get_singleton()->execute("ALTER TABLE orders ADD COLUMN IF NOT EXISTS shipping_method text;");
        DB::Psql::get_singleton()->execute("ALTER TABLE orders ADD COLUMN IF NOT EXISTS shipping_cost numeric(20, 12);");
        DB::Psql::get_singleton()->execute("ALTER TABLE orders ADD COLUMN IF NOT EXISTS total numeric(20, 12);"); // total cost of all items plus shipping costs (sales taxes don't exist in the cryptocurrency world except in the form of tx fees)
        DB::Psql::get_singleton()->execute("ALTER TABLE orders ADD COLUMN IF NOT EXISTS payment_method text;"); // crypto
        DB::Psql::get_singleton()->execute("ALTER TABLE orders ADD COLUMN IF NOT EXISTS currency text;"); // xmr
        DB::Psql::get_singleton()->execute("ALTER TABLE orders ADD COLUMN IF NOT EXISTS notes text;"); // encrypted message containing customer's shipping address
        //DB::Psql::get_singleton()->execute("ALTER TABLE orders ADD COLUMN IF NOT EXISTS column_name column_type;");
    }
    // before we create the order, lets create a savepoint (just in case the order fails or we screw something up)
    ////DB::Psql::get_singleton()->execute("SAVEPOINT order_creation_savepoint;");
    // set order date (timestamp) to the current date (timestamp) // timestamp includes both date and time instead of the time only
    std::string date = DB::Psql::get_singleton()->get_text("SELECT CURRENT_TIMESTAMP;"); // SELECT NOW(), CURRENT_TIMESTAMP; // both now() and current_timestamp are the same thing and both include the timezone: https://dba.stackexchange.com/questions/63548/difference-between-now-and-current-timestamp    // NOW() is postgresql's version; CURRENT_TIMESTAMP is an SQL-standard
    ...
    if(order_id == 0) {
        ...
        ////DB::Psql::get_singleton()->execute("ROLLBACK TO order_creation_savepoint;");
        return;
    }   
    ...
    if(!DB::Psql::get_singleton()->table_exists("order_item")) {
        DB::Psql::get_singleton()->create_table("order_item");
        DB::Psql::get_singleton()->execute("ALTER TABLE order_item ADD COLUMN IF NOT EXISTS order_id integer REFERENCES orders(id) ON DELETE CASCADE;"); // the order_id in which the order_item belongs to
        DB::Psql::get_singleton()->execute("ALTER TABLE order_item ADD COLUMN IF NOT EXISTS item_id integer REFERENCES item(id);");
        DB::Psql::get_singleton()->execute("ALTER TABLE order_item ADD COLUMN IF NOT EXISTS seller_id integer REFERENCES users(id);"); // the seller that is selling the item_id
        DB::Psql::get_singleton()->execute("ALTER TABLE order_item ADD COLUMN IF NOT EXISTS item_qty integer;");
        DB::Psql::get_singleton()->execute("ALTER TABLE order_item ADD COLUMN IF NOT EXISTS item_price numeric(20, 12);"); // the quantity of the item the buyer wishes to buy
    }
    ////////////////////////////////
    /*double subtotal = 0.00, discount = 0.00, shipping_cost = 0.00;
    std::string seller_currency;
    ...
    for(int i = 0; i < cart->get_contents_count(); i++) {
        ...
        unsigned int item_qty = DB::Psql::get_singleton()->get_integer_params("SELECT item_qty FROM cart_item WHERE cart_id = $1 AND item_id = $2", { std::to_string(cart->get_id()), std::to_string(item_id) });
        // if seller_id is not specified (0), then choose a random seller who is selling the same product, but it MUST be in stock!!
        int seller_id = DB::Psql::get_singleton()->get_integer_params("SELECT seller_id FROM inventory WHERE item_id = $1 AND stock_qty > 0", { std::to_string(item_id) });
        if(seller_id == 0) { std::cout << "item seller not found"; DB::Psql::get_singleton()->execute("ROLLBACK TO before_order_creation_savepoint;"); return; }//DB::Psql::get_singleton()->finish(); return; }
        // if the buyer is also the seller XD
        if(user_id == seller_id) {neroshop::print("You cannot buy from yourself", 1); DB::Psql::get_singleton()->execute("ROLLBACK TO before_order_creation_savepoint;"); return;}//DB::Psql::get_singleton()->finish(); return;} // go back to savepoint, before the order was even create to prevent us from inserting a new order
        // get the currency that item is priced in
        seller_currency = DB::Psql::get_singleton()->get_text_params("SELECT currency FROM inventory WHERE item_id = $1 AND seller_id = $2", { std::to_string(item_id), std::to_string(seller_id) });
        // get seller_price
        // if seller does not specify a price for their item, set the item price to its original price
    ...
    for(int i = 0; i < cart->get_contents_count(); i++) {
        neroshop::Item * item = cart->get_item(i);
        std::string item_name = DB::Psql::get_singleton()->get_text_params("SELECT name FROM item WHERE id = $1", { std::to_string(item->get_id()) });//unsigned int item_qty = db.get_column_integer("Cart", "item_qty", "item_id = " + std::to_string(item_id));
        int item_qty = DB::Psql::get_singleton()->get_integer_params("SELECT item_qty FROM cart_item WHERE cart_id = $1 AND item_id = $2", { std::to_string(cart->get_id()), std::to_string(item->get_id()) });
        std::cout << ((i > 0) ? "                              " : "") << "\033[0;94m" + item_name << " (x" << item_qty << ")\033[0m" << std::endl;
        DB::Psql::get_singleton()->execute_params("UPDATE cart_item SET item_qty = $1 WHERE cart_id = $2 AND item_id = $3", { std::to_string(0), std::to_string(cart->get_id()), std::to_string(item->get_id()) }); // reset all item quantity to 0 (now that order has been completed)
    }
    // empty cart after completing order
    cart->empty(user_id);
    // if a user loses internet connection, libcurl cannot get the exchange rate so the order will fail
