...
namespace DB {
//public:
class Psql { // DB is intended for client-server database engine. SQLite does not use a client-server architecture, so unfortunately we cannot solely use SQLite for this as its used for embedding into applications and becomes an integral part of the program
public:
    Psql();
    Psql(const std::string& file_url);
    ~Psql();
...
private:
    PGconn * conn;
    static DB::Psql * db_obj; // singleton obj (to make connection easier for all classes to access and to keep track of a single connection)
};
}
}
