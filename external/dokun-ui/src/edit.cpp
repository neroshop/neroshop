...
Edit::Edit() : color(/*160, 160, 160*/130, 130, 130, 1.0), character_limit(20),
...
void Edit::draw()
{
	...
	on_mouse_press();// edit is pressed, set cursor at position_pressed // set focus if edit is triggered by user input
	...
	//character_limit = width / get_cursor_advance(); // get_cursor_advance() is the space each character in the edit takes up 
    ...
    // Draw children (label, placeholder label, etc.)    //on_draw_after() - deprecated
    ...
    if(label) label->draw(); // default label - will be drawn over placeholder
...
#ifdef DOKUN_X11
    dokun::Window * window = static_cast<dokun::Window *>(Factory::get_window_factory()->get_object(0));
    window->copy_clipboard(label->get_string().c_str());
#endif
}
/////////////
void Edit::paste()
...
{
    if(!label) throw std::runtime_error("label is not initialized");
	// reset edit's text (string, character_data) and cursor (position)
	reset();
	...
	for(int i = 0; i < string0.length(); i++) { // store all string chars in vector
	    // set cursor position at end of string		
		set_cursor_x(cursor_x + cursor_space); // for each character, move 10 units right
        // save each character's data in a vector (for later use)
		int string_index = (cursor_x / cursor_space) - 1;        
	    ...
	    // make sure character_data size does not exceed character_limit
...
void Edit::set_placeholder_text(const std::string& placeholder_text) // text will be gray / almost transparent
{
    ...
    if(label) placeholder_label->set_font(*label->get_font()); // placeholder will use same font as default label
...
void Edit::on_key_press()
{
    ...
    for(unsigned char key = 0; key < 128; key++) //if(dokun::Keyboard::is_down())
	{
		...
		{   
			...
			{
				std::string temp(character_array_to_string());//(label->get_string());//(get_text());		    
			    // calculate string_index (where cursor_x is at)
			    // since we are getting the string_index before increasing cursor_x by 10, increase cursor_x by (cursor_x + cursor_space) to make string_index accurate
			    // ex. cursor_x(10) = string_index(0)
			    // (0 + 10) ÷ 10 - 1 = 0
			    // ex. cursor_x(500) = string_index(49)
			    // (0 + 500) ÷ 10 - 1 = 49
			    // STRING INDEX IS WRONG ON SCROLLING_TEXT
			    // MAYBE USE BOUNDLESS_CURSOR_X?
			    //cursor_x_moved = cursor_x_moved - (temp.size() * cursor_space);
				...
				/*(temp.length() <= get_capacity()) ?
				    (cursor_x + cursor_space) / cursor_space - 1 ://;//((cursor_x + get_cursor_advance()) / get_cursor_advance()) - 1;     // get character at string_index (where cursor_x is) // add cursor_x to increment (10) before we increase cursor_x by 10.
			        // above capacity(50)
			        (boundless_cursor_x + cursor_space) / cursor_space - 1;*/
			    // cursor_x(10) * splits_count
			    // hide characters (for sensative texts like passwords, etc.)
			    if(sensative) secret.insert(string_index, String::to_string(key)); // save actual characters first//
			    std::cout << "secret string: " << secret << std::endl;			    
			    if(sensative) key = '*'; // replace each character with "*"
			    // we won't need temp anymore (soon)
			    // temp is to be displayed
			    // ONLY character_data is to be altered
			    // insert character in string (at a specific index, where the cursor is positioned)
			    ...
			    int cursor_index = cursor_x + cursor_space;//cursor_x;//(temp.length() > get_capacity()) ? boundless_cursor_x : cursor_x;//((label->get_string().length() > character_limit) ? -1 : cursor_x);//cursor_x;
			    ...
			    character_data.insert(character_data.begin() + string_index, std::make_tuple((char)key, cursor_index, string_index)); // this is the initial string_index, not the updated one
			    // no need to save the string_index if characters are inserted into vector in the correct order!// saving the string index is no longer necessary now that the characters are stored in the correct order
			    // but lets save it anyways :?
			    // save and update to the correct the string_index// if cursor_x is not at end of string, assume its either in the beginning or middle of string
			    // to-do: make character_data into a pair instead of a tuple, and remove "string_index" field, storing only the character and its cursor position
			    for(int i = 0; i < character_data.size(); i++)
			        std::get<2>(character_data[i]) = i;
			...
			#endif
			#ifdef DOKUN_DEBUG0
			    // make sure characters are inserted in correct order
			    //std::cout << std::endl << "character_array (as string): " << character_array_to_string() << " (" << character_data.size() << ")" << std::endl;
			    std::cout << std::endl;
			    for(int i = 0; i < character_data.size(); i++) 
			        std::cout << "characters (stored in order): " << std::get<0>(character_data[i]) << " (index:" << i << ")" << ", (string_index:" << std::get<2>(character_data[i]) << ")" << std::endl;
			    std::cout << "characters_count: " << character_data.size() << std::endl;
			#endif
				/////////////////////////////////
				std::cout << std::endl << "\033[0;32mkey (" << (char)key << ") inserted at string_index: " << string_index << " = string_len(" << temp.length() << ") - 1" << "\033[0m" << std::endl;		
			    /////////////////////////////////
		        ...
		        //////////////////////////////////
		        // split the string by capacity(50), n amount of times (n being "splits_count")
		        //std::string temp_split(temp);//std::vector<std::string> temp_split =
		        int start = get_capacity() * splits_count; // increase by capacity(50) every n parts
		        int characters_count = temp.length() - start; // number of characters being drawn at a time
		        std::string temp_split = temp.substr(start, characters_count);
		        std::cout << "\033[1;37;49msplits_count: " << splits_count << " (string_length: " << temp.length() << ", start: " << start << ", last: " << characters_count << "):\033[0m" << std::endl << temp_split << std::endl;
			    /////////////////////////////////
			    // string position (relative to edit)
                int text_x = (characters_count * cursor_space);// = x(140) = characters(14)
			    int borderless_text_x = (temp.length() * cursor_space);// temp.length() should be same as character_data.size()
			    /////////////////////////////////				
	            ...
	            if(temp.length() > character_limit) temp.resize(character_limit); // resize string to whatever the character limit is - Character limit is user-defined
	            if(temp.length() > get_capacity()) // if text goes past edit // (set_cursor_x() already checks if cursor goes past edit, so we don't have to worry about that)
	            {   // resize text to fit edit
	                ////if(!multilined) temp.resize(get_capacity()); // capacity is basically the number of strings that can be drawn on top of an edit based on the edit's width // capacity = width / get_cursor_advance() = x characters MAX
		            //////////////////
		            // if string_length is more than the capacity
		            // cursor_x must be from 0 to 500 (don't use boundless cursor_x)
		            // string_index must be correct (use boundless)
		            // ** this is for non-multilined text **
                    std::cout << DOKUN_UI_TAG "\033[0;33mCapacity (" <<  get_capacity() << ") has been surpassed by text\033[0m" << std::endl;
		            // move cursor_x to end of edit (just leave it as is)
		            // as long as text (length) surpasses capacity (edit's width), cursor_x will remain at end of edit's width (cursor cannot go past edit width)
		            //set_cursor_x(cursor_x - cursor_space);//cursor_x = get_width();
		            // the real value of cursor_x
		            ////int temp_cursor_x = (cursor_x + cursor_space);
		            ////std::cout << "cursor_x (current): " << temp_cursor_x << std::endl;		            
		            // DETERMINE THE BOUNDLESS CURSOR_X POSITION
		            // SO THAT WE CAN USE THIS INFO TO GET THE STRING_INDEX
		            //boundless_cursor_x = (character_data.size() * cursor_space); // no character data pushed yet, so add cursor_offset (+10)
		            //std::cout << "cursor_x (boundless): " << fabs(boundless_cursor_x) << std::endl;
		            // OUR GOAL IS TO GET THE NON-RELATIVE POSITION OF CURSOR (SAY, THE CURSOR HAD NO LIMITS AND COULD GO BEYOND EDIT'S SCOPE)
		            ////// FIND A WAY TO CONVERT A BOUNDLESS CURSOR_X: 510, 520, 530 etc. to 10, 20, 30, etc.    
		            // how far the cursor_x (boundless) can go in edit
		            ////int max_cursor_x = character_limit * cursor_space; // 128 x 10 = 1280 (cursor can only move up 1280 units)
                    ////std::cout << "max_cursor_x (boundless): " << max_cursor_x << std::endl;//std::cout << "maximum_cursor_x: " << max_cursor_x << std::endl;
		            //int num_breaks = round(character_data.size() / get_capacity()); //get_capacity() / character_data.size()
		            // (cursor_x)510 - 500(capacity) = 10 (actual cursor_x)
		            // (cursor_x)520 - 500(capacity) = 20 (actual cursor_x)
		            // (cursor_x)1280 - 500(capacity) => gives wrong result
		            // (character_x)1280 − (500 * (1280 / 500)) // cursor position zero
		            //character_limit/*(128)*/ * cursor_space/*10*/ // = 1280 (new cursor_x)
		            // CONVERT BOUNDLESS_CURSOR_X(510) TO BOUNDED_CURSOR_X(10)
		            // SPLIT THE STRING WHEN IT EXCEEDS CAPACITY
		            // find a way to keep range between 0-500, but at a constant value of 10
		            // when the new_cursor_x == 0, it means we have reached the max length or character_limit
		            // (new_cursor_x == 0) ? temp.length() * cursor_space : new_cursor_x;
		            ////int trimmed_cursor_x = text_x;//temp_cursor_x - (get_width() * (fake_max_cursor_x / get_width()));//fake_cursor_x - get_capacity();//(fake_cursor_x > 500[edit_width])//fake_cursor_x - get_capacity();
		            ////std::cout << "text_x (relative to edit): " << trimmed_cursor_x << std::endl;
		            ////std::cout << "borderless_text_x: " << borderless_text_x << std::endl;
		            // now calculate current cursor position using the text coordinates
		            // calculate string_index
		            // MAXIMUM STRING INDEX MUST RANGE FROM 0 TO (CHARACTER_LIMIT - 1)
		            ////int string_index_count = (fake_cursor_x / cursor_space) - 1;//temp.length() - 1;
		            ////std::cout << "string_index_count: " << string_index_count << std::endl;
		            // todo: find a way to get actual string index
		            // FIND A WAY TO CALCULATE CURRENT STRING INDEX WHERE CURSOR IS LOCATED
		           //// std::cout << "string_index (at cursor_x): " << string_index << std::endl;
		            // show only the most recent 50 characters (result should equal 50)
		            // string_length(83) - characters_count(33) = 50 // should ALWAYS equal the capacity(50)
		            // from end of temp_split to ?? = 50 visible characters to draw
		            //std::string temp_recent = temp.substr(start - (temp.length() - character_count) , get_capacity()/*50 characters to draw!!*/);
		            // show a part of the label
		            // from end of string to wherever it adds up to 50
		            //std::string temp_recent = temp.substr(temp.end() - std::min(text.size(), get_capacity()/*50*/), text.end());//.substr(temp.length() - std::min(text.size(), get_capacity()) )
			        // move boundless_cursor_x too
			        //if(splits_count > 0) // same as temp_length > capacity
			        ////boundless_cursor_x = (character_data.size() * cursor_space) - (characters_count * cursor_space) + cursor_space;// + cursor_x; // temp.length() should be the same as characters_count I think			        
			        // draw characters from where cursor is (cursor_x)		            
		            // draw and show a section or part of the label where the length of the string is equal to the edit's capacity(50)
		        ////label->set_string(temp_split);
		            // move cursor to end of text
		            //set_cursor_x(text_x);//(cursor_x + text_x)	
			        ////cursor_x = boundless_cursor_x - text_x
			        //set_cursor_x((boundless_cursor_x - text_x)); //* (splits_count)// + 1)
			        //for(int i = 0; i < character_data.size(); i++) {
			        //if find_first_of(temp.split) in character_data vector then
			        //std::get<1>(character_data[i]) = i; // cursor_x
			        //}		            	            
		            /////////////////
		            //if(multilined)  // if multilined=true, create a new line // a multilined editor
		            //{}
	            ...
	            /////////////////////////////////
	            // and finally, set the string
	            // (temp) this only works for edits whose label can go past the edit's bounds (edit_width)
				// (temp_split) will show a section or part of the label where the length of the string is equal to the edit's capacity(50)
				// if string has not gone past edit's bounds (width) then draw it as we normally would
			////if(temp.length() <= get_capacity()) label->set_string(temp);
			    // CURSOR_X RANGES FROM 0-500 WHILE BOUNDLESS_CURSOR_X RANGES FROM 0-(CHARACTER_LIMIT * CURSOR_SPACE)
			    /////////////////////////////////
                // move cursor_x 10 units for each character	
				////set_cursor_x(cursor_x + cursor_space);//if(splits_count < 1) set_cursor_x(cursor_x + cursor_space); // label_width does not give accurate results so DO NOT use! Use 10 instead.
			    // do same for boundless_cursor_x
			    if(splits_count < 1) { // same as: if(temp.length() <= get_capacity())
		            ...
		            *//////////////////////////////
		            ...
		            boundless_cursor_x = (character_data.size() * cursor_space) - (characters_count * cursor_space) + cursor_x;//boundless_cursor_x = boundless_cursor_x + cursor_space;
		            /////////////////////////////
			    ...
			    if(splits_count > 0) { // same as: if(temp.length() > get_capacity()) // if text goes past edit
		            ...
		            // if you want to insert a character in the middle of text
		            // then cursor_x must stay still and NOT move at all
		            //////////////////////////////
		            // update the label when cursor position is changed
			    ...
			    // make sure cursor does not go past edit's width
		        //int max_cursor_x = character_limit * cursor_space;
		        //if(cursor_x >= max_cursor_x) cursor_x = max_cursor_x;
		        //}
		        //if(temp.length() > get_capacity()) cursor_x = text_x;
		        // reset cursor if it goes outside edit bounds
		        // maybe show characters closer to cursor_x?? I dunno ...
		        // draw characters from where cursor is (cursor_x)
		        // show only last n characters (n being capacity)
		        /*// get last n characters from string (with n being the edit's capacity)// capacity = total number of characters that can be shown on the edit at a time// IF TEXT GOES PAST EDIT'S WIDTH, ONLY SHOW THE LAST CHARACTERS(size: get_capacity()) in character_array vector
	            int n = (temp.length() < get_capacity()) ? temp.length() : get_capacity();
	            std::vector<char> last_n_characters(temp.end() - n, temp.end()); // of course this will crash and burn if x.size() < n I think?
	            // draw and show only the last n characters in edit (visible text)
	            temp = std::string(last_n_characters.begin(), last_n_characters.end());
		        if(temp.size() >= get_capacity()) std::cout << "(draw): " << temp << " (" << temp.size() << ")" << std::endl;*/
	            // we know that only 50(capacity) characters can be drawn at a time on the edit
	            // what if, the string_length grows to 51 ?
	            // then we must reduce the string_length from where cursor_x is
	            // string_length(51) - capacity(50) = 1
	            // reduce string_length from where cursor_x is
	            // get how much the string_length has increased in length (from its original capacity)
	            // also, hide the extra characters
	            // int extra_characters_count = string_length - capacity;    //if(string_increment <= 0) string_increment = 0; // in case its negative
	            // n = extra_characters_count - string_index
	            // from string_index(at cursor_x) to where it maxes out to 50 characters in length

			    /////////////////////////////////
			    //std::vector<char> hidden_characters
			    // for every key (character) added after capacity(50) exceeded, hide some character
			    // but which characters to hide? - hide characters by (string_increment) from where cursor_x is
			    // new_cursor_x = cursor_x - (extra_characters_count * cursor_space); // or hidden_characters_count or string_added
			    // WHEREEVER THE CURSOR_X GOES, SHIFT TEXT
			    /////////////////////////////////
			}
		}
	}
...
void Edit::on_backspace() // bug: crashes everytime backspace is used when cursor position is not at the end of text
{
...
#endif
	{
	    ...
	    if(string_index < 0) return;//{ std::cout <<"\033[31mstring index is negative. Exiting on_backspace() ...\033[0m" << std::endl;return;} // if string_index is negative, exit function since we cannot delete a negative string index. String indexes should start from 0 // a string index at -1 means that the cursor is at position 0 relative to the edit, so it cannot go back any further or else it will go outside the edit's bounds [2021-09-20]
		...
		// erase last character in string (at a specific index, where the cursor is positioned)    //if(get_text().length() > 0) set_text(get_text().erase(get_text().length() - 1));	// old way of deleting characters
		#ifdef DOKUN_DEBUG
	        ...
	        if(sensative) secret.erase(secret.begin() + string_index); // erase secret string too //
	        std::cout << "secret string: " << secret << std::endl;		
		    // temporary string - won't be necessary anymore
		    // only character_data is important
		    std::string temp(character_array_to_string());//(label->get_string());//(get_text());            // copy of label_string	    
		    ...
		    ////set_cursor_x(cursor_x - cursor_space);
			// do same for boundless_cursor_x
			if(splits_count < 1) { // no more splits left // same as: if(temp.length() <= get_capacity())
                set_cursor_x(cursor_x - cursor_space);
			    ...
			    // if no splits left, boundless_cursor_x will follow cursor_x
			    boundless_cursor_x = (character_data.size() * cursor_space) - (temp.length() * cursor_space) + cursor_x;//boundless_cursor_x = boundless_cursor_x + cursor_space;//(start * cursor_space) + cursor_x;
			...
			if(splits_count > 0) { // multiple splits // same as: if(temp.length() > get_capacity())
			    boundless_cursor_x = boundless_cursor_x - cursor_space;//set_cursor_x(boundless_cursor_x + text_x);//(text_x);
                // make sure boundless_cursor_x is NEVER negative, can NEVER be negative anyways
                if(boundless_cursor_x <= 0) boundless_cursor_x = 0;
...
void Edit::on_placeholder_image() {/*
	...
	if(placeholder_image->get_alignment() == "left"  ) { placeholder_image->set_relative_position(0, 0);	}
	if(placeholder_image->get_alignment() == "center") { placeholder_image->set_relative_position((get_width() - image_width) / 2, (get_height() - image_height) / 2);}
	if(placeholder_image->get_alignment() == "right" ) { placeholder_image->set_relative_position(get_width() - image_width, 0);	}
	...
	placeholder_image->scale_to_fit(get_width(), get_height());
	...
	placeholder_image->draw(); // draw*/
}
/////////////
void Edit::on_sensative_set() { // or on_sensativity_set ?
    // if label is not empty (sensative ONLY), meaning that set_text() was called before set_sensative()
    if(!sensative) return;
    if(!label->is_empty()) {
        // save (store) secret string
        secret = label->get_string();//std::cout << "secret saved: " << secret << std::endl;
